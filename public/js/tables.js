...
$("#len_rows").on("change", function () {
  ...
  var len = this.value;
  var params = new URLSearchParams(search ? search.substr(1, length) : "");
  if (len) params.set("len", len);else params["delete"]("len");
  var str_params = params.toString();
  var url = location.origin + location.pathname + (str_params ? '?' + str_params : "");
  console.log(url);
  location.href = url;
